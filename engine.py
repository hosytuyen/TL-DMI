...
def train_reg(args, model, criterion, optimizer, trainloader, testloader, n_epochs, model_path, device='cuda'):
    best_ACC = (0.0, 0.0)
    if args["dataset"]["model_name"] == "Test":
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=args[args["dataset"]["model_name"]]["adjust_epochs"], gamma=0.1)
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % 10 == 0:
            torch.save({'state_dict':model.state_dict()}, os.path.join(model_path, "{}_{}.tar").format(args["dataset"]["model_name"], epoch))

    print("Best Acc:{:.2f}".format(best_ACC[0]))
    return best_model, best_ACC

def train_vib(args, model, criterion, optimizer, trainloader, testloader, n_epochs, model_path, device='cuda'):
	best_ACC = (0.0, 0.0)
...
def train_specific_gan(cfg):
    ...
    if cfg["train"]["GAN_resume"] is not "":
        ckp_G = torch.load(cfg["train"]["GAN_resume"])
